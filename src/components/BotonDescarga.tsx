...
export default function BotonDescarga({ id, formato }: { id: string; formato: "excel" | "pdf" }) {
  ...
  useEffect(() => {
    if (formato === "pdf") cuandoEsteLibre(() => import("@/services/pdf-navegador").then((m) => m.precargarPdf()).catch(() => {}));
  }, [formato]);
  ...
  async function generar() {
    ...
    try {
      ...
      if (esVersionDesactualizada(e)) {
        if (window.confirm(`${MENSAJE_VERSION}\n\n¿Recargar ahora?`)) window.location.reload();
      } else {
